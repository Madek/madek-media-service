(ns madek.media-service.utils.history
...
(defn ignore-anchor-click?
  ...
  (let [current-domain (if (exists? js/location)
                         ^String (.getDomain (.parse Uri js/location)))]
    (and (or (and (not ^Boolean (.hasScheme uri)) (not ^Boolean (.hasDomain uri)))
             (= current-domain ^String (.getDomain uri)))
         (not (.-altKey e))
         (not (.-ctrlKey e))
         (not (.-metaKey e))
         (not (.-shiftKey e))
         ...
         (reitit/match-by-path router ^String (.getPath uri)))))
                                ...
                                (when-let [el (closest-by-tag (event-target e) "a")]
                                  (let [uri (.parse Uri (.-href el))]
                                    (when (ignore-anchor-click-predicate router e el uri)
                                      (.preventDefault e)
                                      (let [path (str ^String (.getPath uri)
                                                      (when ^Boolean (.hasQuery uri)
                                                        (str "?" (.getQuery uri)))
                                                      (when ^Boolean (.hasFragment uri)
                                                        (str "#" ^String (.getFragment uri))))]
